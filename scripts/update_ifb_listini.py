...
def compute_row(branch, code, sale_slots, purch, item_card=None, transport_costs=None, iss_carriage=None):
    ...
    # Se DAP=0 e FCA>0: usa carriage da ISS (BC per-articolo) poi fallback tabella trasporti
...
if __name__ == "__main__":
    ...
    # Fetch ISS per TUTTI gli articoli: carriage per-articolo da BC + articoli HK non nel listino
    iss_prices = {}
    iss_carriage = {}
    try:
        ...
        # Fetch ISS per tutti i codici (per carriage) + HK mancanti (per prezzo)
        all_target = all_purchase_codes | hk_codes
        print(f"  Fetch ISS per {len(all_target)} articoli (carriage per-articolo + HK mancanti)...")
